...
local function CreateItem(tbl,width,height,Flags,Buttons,Title,ExecDelay)
  ...
  local fg_dlg=band64(tFarColor,0xF)
  local bg_dlg=bit64.rshift(tFarColor,4)
  ...
  for y=1,tbllen do
    ...
    local xlen = tbl[y]:len()
    for x=1,X4 do
      local char = x <= xlen and tbl[y]:sub(x,x) or " "
      ...
      buffer[ptr]={Char=char,Attributes=bor64(fg, bit64.lshift(bg,4))}
      ptr=ptr+1
    end
  end
