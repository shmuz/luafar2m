...
local noshowdir = mf.mload(dbconfig, dbshowdir) or false;

local menu = function(pattern)
    local entry = function(e, space)
        return {text = e.alias .. (" "):rep(space + 1 - e.alias:len()) .. (not noshowdir and e.path or ""); entry = e};
    end;
    ...
    local best = function(pattern)
        local scan = function(pattern)
            ...
            for i, v in ipairs(load_nf()) do -- filter items by pattern & calculate max width alias
                if (not pattern or v.alias:lower():match(pattern:lower())) then
                    table.insert(ent, v);
                    space = math.max(space, v.alias:len());
                    if (pattern and pattern:lower() == v.alias:lower()) then return 1, v; end; -- exact matching - return it
                end;
            end;
            return ent, space;
        end;
    ...
    for i, v in ipairs(ent) do table.insert(entries, entry(v, space)); end;

    if (pattern and table.getn(entries) == 1) then return 1, entries[1].entry; end;
    if (pattern and table.getn(entries) == 0) then return nil; end;
    ...
    local F = far.Flags
    local item, position = far.Menu({Title = "Named Folders Lua Edition"; Bottom = bottom,
                                     Flags = bit.bor(F.FMENU_AUTOHIGHLIGHT, F.FMENU_WRAPMODE)},
        entries, brakes);

    if (not item) then return nill;
...
local newentry = function(entry)
    ...
    local guid = "{8B0EE808-C5E3-44D8-9429-AAFD8FA04067}";
    local F = far.Flags
    local items = {
    --[[ 1]] {F.DI_DOUBLEBOX, 3,1, 65,8, 0, 0,0, 0, "Named Folder"},
    --[[ 2]] {F.DI_TEXT,      5,2, 14,2, 0, 0,0, 0, "&Alias name:"},
    --[[ 3]] {F.DI_EDIT,      5,3, 63,3, 0, 0,0, 0, entry and entry.alias or table.remove(split(panel.GetPanelDirectory(nil, 1).Name, "[^\\/]+"))},
    --[[ 4]] {F.DI_TEXT,      5,4, 11,4, 0, 0,0, 0, "&Target:"},
    --[[ 5]] {F.DI_EDIT,      5,5, 63,5, 0, 0,0, 0, entry and entry.path or panel.GetPanelDirectory(nil, 1).Name},
...
local action = function(text)
    ...
    while(true) do
        ...
        elseif (tostring(res) == "showdir") then
            ...
            mf.msave(dbconfig, dbshowdir, showdir);
        end;
    end;
end;
