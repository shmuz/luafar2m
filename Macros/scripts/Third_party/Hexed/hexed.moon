...
HexDraw=(hDlg,data)->
  ...
  GetChar=(pos)->
    newchar=format '%02X',byte data.chunk,pos
    newchar,data.edit and (format '%02X',byte data.oldchunk,pos) or newchar
  ...
  with data
    ...
    for row=0,.height-1
      ...
      for col=1,16
        pos=col+row*16
        if pos<=len
          newchar,oldchar=GetChar pos
          txtl=pos==.cursor and not .edit and .textel_sel or
            (newchar==oldchar and .textel or .textel_changed)
          DrawStr row*.width+(col-1)*3+1+12+(col>8 and 2 or 0),newchar,txtl
    ...
    if .edit
      ...
      newchar,oldchar=GetChar cursor
      .editchanged=newchar~=oldchar
      hDlg\SetText _edit,.editascii and (.displaytext\sub cursor,cursor) or
        sub newchar,.editpos+1,.editpos+1
