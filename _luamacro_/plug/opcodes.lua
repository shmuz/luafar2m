return {
  ...
  MCODE_F_ENVIRON=0x80C0D; -- S=env(S)
  ...
  MCODE_F_MIN=0x80C1C; -- N=min(N1,N2)
  MCODE_F_MOD=0x80C1D; -- N=mod(a,b) == a % b
  MCODE_F_MLOAD=0x80C1E; -- B=mload(var)
  MCODE_F_MSAVE=0x80C1F; -- B=msave(var)
  MCODE_F_MSGBOX=0x80C20; -- N=msgbox(["Title"[,"Text"[,flags]]])
  MCODE_F_PANEL_FATTR=0x80C21; -- N=Panel.FAttr(panelType,fileMask)
  MCODE_F_PANEL_SETPATH=0x80C22; -- N=panel.SetPath(panelType,pathName[,fileName])
  MCODE_F_PANEL_FEXIST=0x80C23; -- N=Panel.FExist(panelType,fileMask)
  MCODE_F_PANEL_SETPOS=0x80C24; -- N=Panel.SetPos(panelType,fileName)
  MCODE_F_PANEL_SETPOSIDX=0x80C25; -- N=Panel.SetPosIdx(panelType,Idx[,InSelection])
  MCODE_F_PANEL_SELECT=0x80C26; -- V=Panel.Select(panelType,Action[,Mode[,Items]])
  MCODE_F_PANELITEM=0x80C27; -- V=PanelItem(Panel,Index,TypeInfo)
  MCODE_F_EVAL=0x80C28; -- N=eval(S[,N])
  MCODE_F_RINDEX=0x80C29; -- S=rindex(S1,S2[,Mode])
  MCODE_F_SLEEP=0x80C2A; -- Sleep(N)
  MCODE_F_STRING=0x80C2B; -- S=string(V)
  MCODE_F_SUBSTR=0x80C2C; -- S=substr(S,start[,length])
  MCODE_F_UCASE=0x80C2D; -- S=ucase(S1)
  MCODE_F_WAITKEY=0x80C2E; -- V=waitkey([N,[T]])
  MCODE_F_XLAT=0x80C2F; -- S=xlat(S)
  MCODE_F_FLOCK=0x80C30; -- N=FLock(N,N)
  MCODE_F_CALLPLUGIN=0x80C31; -- V=callplugin(SysID[,param])
  MCODE_F_REPLACE=0x80C32; -- S=replace(sS,sF,sR[,Count[,Mode]])
  MCODE_F_PROMPT=0x80C33; -- S=prompt("Title"[,"Prompt"[,flags[, "Src"[, "History"]]]])
  MCODE_F_BM_ADD=0x80C34; -- N=BM.Add()  - добавить текущие координаты и обрезать хвост
  MCODE_F_BM_CLEAR=0x80C35; -- N=BM.Clear() - очистить все закладки
  MCODE_F_BM_DEL=0x80C36; -- N=BM.Del([Idx]) - удаляет закладку с указанным индексом (x=1...), 0 - удаляет текущую закладку
  MCODE_F_BM_GET=0x80C37; -- N=BM.Get(Idx,M) - возвращает координаты строки (M==0) или колонки (M==1) закладки с индексом (Idx=1...)
  MCODE_F_BM_GOTO=0x80C38; -- N=BM.Goto([n]) - переход на закладку с указанным индексом (0 --> текущую)
  MCODE_F_BM_NEXT=0x80C39; -- N=BM.Next() - перейти на следующую закладку
  MCODE_F_BM_POP=0x80C3A; -- N=BM.Pop() - восстановить текущую позицию из закладки в конце стека и удалить закладку
  MCODE_F_BM_PREV=0x80C3B; -- N=BM.Prev() - перейти на предыдущую закладку
  MCODE_F_BM_BACK=0x80C3C; -- N=BM.Back() - перейти на предыдущую закладку с возможным сохранением текущей позиции
  MCODE_F_BM_PUSH=0x80C3D; -- N=BM.Push() - сохранить текущую позицию в виде закладки в конце стека
  MCODE_F_BM_STAT=0x80C3E; -- N=BM.Stat([M]) - возвращает информацию о закладках, N=0 - текущее количество закладок	MCODE_F_TRIM,                     // S=trim(S[,N])
  MCODE_F_TRIM=0x80C3F; -- S=trim(S[,N])
  MCODE_F_FLOAT=0x80C40; -- N=float(V)
  MCODE_F_TESTFOLDER=0x80C41; -- N=testfolder(S)
  MCODE_F_PRINT=0x80C42; -- N=Print(Str)
  MCODE_F_MMODE=0x80C43; -- N=MMode(Action[,Value])
  MCODE_F_EDITOR_SETTITLE=0x80C44; -- N=Editor.SetTitle([Title])
  MCODE_F_MENU_GETVALUE=0x80C45; -- S=Menu.GetValue([N])
  MCODE_F_MENU_ITEMSTATUS=0x80C46; -- N=Menu.ItemStatus([N])
  MCODE_F_BEEP=0x80C47; -- N=beep([N])
  MCODE_F_KBDLAYOUT=0x80C48; -- N=kbdLayout([N])
  MCODE_F_WINDOW_SCROLL=0x80C49; -- N=Window.Scroll(Lines[,Axis])
  MCODE_F_GETOPTIONS=0x80C4A; -- N=GetOptions()
  MCODE_F_KEYMACRO=0x80C4B; 
  ...
  MCODE_V_ITEMCOUNT=0x80824; -- ItemCount - число элементов в текущем объекте
  MCODE_V_CURPOS=0x80825; -- CurPos - текущий индекс в текущем объекте
  MCODE_V_TITLE=0x80826; -- Title - заголовок текущего объекта
  MCODE_V_HEIGHT=0x80827; -- Height - высота текущего объекта
  MCODE_V_WIDTH=0x80828; -- Width - ширина текущего объекта
  MCODE_V_EDITORFILENAME=0x80829; -- Editor.FileName - имя редактируемого файла
  MCODE_V_EDITORLINES=0x8082A; -- Editor.Lines - количество строк в редакторе
  MCODE_V_EDITORCURLINE=0x8082B; -- Editor.CurLine - текущая линия в редакторе (в дополнении к Count)
  MCODE_V_EDITORCURPOS=0x8082C; -- Editor.CurPos - текущая поз. в редакторе
  MCODE_V_EDITORREALPOS=0x8082D; -- Editor.RealPos - текущая поз. в редакторе без привязки к размеру табуляции
  MCODE_V_EDITORSTATE=0x8082E; -- Editor.State
  MCODE_V_EDITORVALUE=0x8082F; -- Editor.Value - содержимое текущей строки
  MCODE_V_EDITORSELVALUE=0x80830; -- Editor.SelValue - содержит содержимое выделенного блока
  MCODE_V_DLGITEMTYPE=0x80831; -- Dlg.ItemType
  MCODE_V_DLGITEMCOUNT=0x80832; -- Dlg.ItemCount
  MCODE_V_DLGCURPOS=0x80833; -- Dlg.CurPos
  MCODE_V_DLGINFOID=0x80834; -- Dlg.Info.Id
  MCODE_V_VIEWERFILENAME=0x80835; -- Viewer.FileName - имя просматриваемого файла
  MCODE_V_VIEWERSTATE=0x80836; -- Viewer.State
  MCODE_V_CMDLINE_ITEMCOUNT=0x80837; -- CmdLine.ItemCount
  MCODE_V_CMDLINE_CURPOS=0x80838; -- CmdLine.CurPos
  MCODE_V_CMDLINE_VALUE=0x80839; -- CmdLine.Value
  MCODE_V_DRVSHOWPOS=0x8083A; -- Drv.ShowPos - меню выбора дисков отображено: 1=слева (Alt-F1), 2=справа (Alt-F2), 0="нету его"
  MCODE_V_DRVSHOWMODE=0x8083B; -- Drv.ShowMode - режимы отображения меню выбора дисков
  MCODE_V_HELPFILENAME=0x8083C; -- Help.FileName
  MCODE_V_HELPTOPIC=0x8083D; -- Help.Topic
  MCODE_V_HELPSELTOPIC=0x8083E; -- Help.SelTopic
  MCODE_V_MENU_VALUE=0x8083F; -- Menu.Value
}
