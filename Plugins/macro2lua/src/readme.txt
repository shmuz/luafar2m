...
Конвертер выполнен как плагин для Far3/far2m, т.к. используется фаровская
функция для верификации имён клавиш. Плагин использует библиотеку LuaFAR,
которая поставляется вместе с Far3/far2m.

Плагин обрабатывает INI-файлы far2l (key_macros.ini), XML-файлы формата
экспорта Far3 и FML-файлы плагина MacroLib. При успешном преобразовании,
плагин создаёт выходной файл.
...
<syntax> указывает синтаксис входного файла, может быть одним из:
        ini_file       -- far2l INI файл (значение по умолчанию)
...
-----------------
        local id = 0x50B1ABE5
        str_out, str_message = Plugin.Call(id, str_syntax, str_in)
