...
local function test_bug1_20111114 (lib)
  ...
  if not bSelectFound then _Plugin.History.config.bSelectFound = true end

  SetEditorText("Д121") -- 1-st char takes 2 bytes in UTF-8

  local dt = { sSearchPat="1", sOrigin="cursor", CurPos=3 }
  RunEditorAction(lib, "test:search", dt, 1, 0)
  if not bSelectFound then _Plugin.History.config.bSelectFound = false end
...
local function test_bug2_20111114 (lib)
  ...
  editor.Select("BTYPE_STREAM",1,1,4,1)
  RunEditorAction(lib, "test:search", dt, 1, 0)
  if not bSelectFound then _Plugin.History.config.bSelectFound = false end
  ...
  dt.CurPos = 1
  editor.Select("BTYPE_STREAM",1,1,3,1)
...
local function test_bug_20120301 (lib)
  ...
  local pos = editor.RealToTab(1, 3)
  editor.Select("BTYPE_COLUMN", 1, pos, 3, 1)
  RunEditorAction(lib, "test:replace", dt, 1, 1)
end
...
local TestDir = "/tmp/LFSearch_Test"
local CurDir = assert(panel.GetPanelDirectory(1))
...
local function test_one_mask (mask, files, num)
  ...
  PrAssert(panel.SetPanelDirectory(1, TestDir))
...
local function test_dir_filter()
  ...
  PrAssert(panel.SetPanelDirectory(1, root_dir))
